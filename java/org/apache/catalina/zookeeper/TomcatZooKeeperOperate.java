...
public class TomcatZooKeeperOperate {

    private static final Log logger = LogFactory.getLog(TomcatZooKeeperOperate.class);

    TomcatZooKeeperListener myZooKeeper = new TomcatZooKeeperListener(1);
    ...
    public boolean createZNode(String path, String data){
        try {
            String zkPath = TomcatZooKeeperListener.zooKeeper.create(path, data.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
    ...
    public boolean deteleZNode(String path){
        try {
            TomcatZooKeeperListener.zooKeeper.delete(path, -1);
    ...
    public boolean updateZNodeData(String path,String data){
        try {
            Stat stat = TomcatZooKeeperListener.zooKeeper.setData(path, data.getBytes(), -1);
    ...
    public String readData( String path ){
        String data = null;
        try {
            data = new String( TomcatZooKeeperListener.zooKeeper.getData( path, false, null ) );
    ...
    public List<String> getChild( String path ){
        try{
            List<String> list=TomcatZooKeeperListener.zooKeeper.getChildren( path, false );
            if(list.isEmpty()){
    ...
    public boolean isExists( String path ){
        try {
            Stat stat = TomcatZooKeeperListener.zooKeeper.exists( path, false );
            return null != stat;
        } catch (KeeperException e) {
            logger.error( "读取数据失败,发生KeeperException! path: " + path
                    + ", errMsg:" + e.getMessage(), e );
